   ...
   jobs:
     update:
       ...
       steps:
         ...
         - name: 오늘의 문제 뽑기
           # push(=풀이 커밋)에는 재추천 안 함 → 자정·수동 리롤에서만 (풀이 시 추천 흔들림 방지)
...
4. **끝.** 🌱 잔디는 `push` 트리거로 **셋업/풀이 커밋 직후(≈1분)** 갱신됩니다(직전 잠깐 깨진 이미지 가능).
   🎲 오늘의 문제는 **풀이 커밋(push)엔 재추천하지 않고** 자정 또는 수동 리롤에서만 바뀝니다
   (풀이 시 추천이 흔들리지 않도록). 셋업 직후 바로 보려면 Actions → Run workflow.
